...
class viz:
	...
	def __init__(self, pathA, pathB, use_GPU = False):
		...
		#debug- this may allow alphas to be turned on
		# glEnable(GL_BLEND)
	...
	def on_draw(self):
		...
		xl3A = xElbA + ( self.l2 * np.sin(link0RotA*(np.pi/180))*np.sin(link2RotEffA*(np.pi/180)))
		yl3A = yElbA + ( self.l2 * np.cos((link2RotEffA*(np.pi/180)))) 
		zl3A = zElbA + ( self.l2 * np.cos(link0RotA*(np.pi/180))*np.sin(link2RotEffA*(np.pi/180)))
		xl4A = xElbA + ( (self.l2+self.l3) * np.sin(link0RotA*(np.pi/180))*np.sin(link2RotEffA*(np.pi/180)))
		yl4A = yElbA + ( (self.l2+self.l3) * np.cos((link2RotEffA*(np.pi/180)))) 
		zl4A = zElbA + ( (self.l2+self.l3) * np.cos(link0RotA*(np.pi/180))*np.sin(link2RotEffA*(np.pi/180)))
		...
		xl3B = xElbB + ( self.l2 * np.sin(link0RotB*(np.pi/180))*np.sin(link2RotEffB*(np.pi/180)))
		yl3B = yElbB + ( self.l2 * np.cos((link2RotEffB*(np.pi/180)))) 
		zl3B = zElbB + ( self.l2 * np.cos(link0RotB*(np.pi/180))*np.sin(link2RotEffB*(np.pi/180)))
		xl4B = xElbB + ( (self.l2+self.l3) * np.sin(link0RotB*(np.pi/180))*np.sin(link2RotEffB*(np.pi/180)))
		yl4B = yElbB + ( (self.l2+self.l3) * np.cos((link2RotEffB*(np.pi/180)))) 
		zl4B = zElbB + ( (self.l2+self.l3) * np.cos(link0RotB*(np.pi/180))*np.sin(link2RotEffB*(np.pi/180)))
		...
		self.draw_link2(self.link2, xElbB, yElbB, zElbB, link0RotB, link1RotB, link2RotB, wireframe = True)
...
if __name__ == "__main__":
	...
	viz = viz(path2, path1, use_GPU=False)
