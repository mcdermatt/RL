...
ankle_frame = ReferenceFrame('Af')
lower_leg_frame = ReferenceFrame('L')
upper_leg_frame = ReferenceFrame('U')
torso_frame = ReferenceFrame('T')

slip, theta1, theta2, theta3 = dynamicsymbols('slip, theta1, theta2, theta3')

#Reference frames - rotation only
#orient lower leg frame with respect to inertial (base) frame
ankle_frame.orient(inertial_frame, 'Axis', (0, inertial_frame.z))
lower_leg_frame.orient(ankle_frame, 'Axis', (theta1, ankle_frame.z))
...
start = Point('St')
ankle = Point('A')
ankle.set_pos(start, slip*inertial_frame.x)
# ankle.set_pos(start, 1*inertial_frame.x)
...
dSlip, omega1, omega2, omega3 = dynamicsymbols('dSlip, omega1, omega2, omega3')
kinematical_differential_equations = [dSlip - slip.diff(),
                                      omega1 - theta1.diff(),
                                      omega2 - theta2.diff(),
                                      omega3 - theta3.diff()]
...
# set ankle to be sliding?
ankle.set_vel(inertial_frame, 1*inertial_frame.x)
...
#ankle frictional force
fric_force = (ankle, -0.01*dSlip*inertial_frame.x)
...
coordinates = [slip, theta1, theta2, theta3]
speeds = [dSlip, omega1, omega2, omega3]
...
start_shape = Sphere(color ='red', radius = 0.25)
...
start_viz_frame = VisualizationFrame(inertial_frame, start, start_shape)
...
scene = Scene(inertial_frame, start)
